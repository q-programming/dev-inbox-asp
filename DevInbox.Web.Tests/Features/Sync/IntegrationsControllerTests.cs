...
public class IntegrationsControllerTests
{
    ...
    {
        var body = new ConnectPatRequest { Token = "ado_pat", ExpiresAt = DateTimeOffset.UtcNow.AddDays(14), Organization = "contoso" };
        var expected = new IntegrationDto { Id = 7, Type = IntegrationType.Ado, Status = DevInbox.Web.Infrastructure.OpenApi.Generated.IntegrationStatus.ACTIVE, Organization = "contoso" };
        _adoIntegrationService.ConnectPatAsync(UserId, body.Organization!, body.Token, body.ExpiresAt, Arg.Any<CancellationToken>()).Returns(expected);

        var result = await _controller.ConnectAdoPatAsync(body);

        Assert.Same(expected, result);
        await _adoIntegrationService.Received(1).ConnectPatAsync(UserId, body.Organization!, body.Token, body.ExpiresAt, Arg.Any<CancellationToken>());
    ...
    [Fact(DisplayName = "ConnectAdoPatAsync should reject a missing organization")]
    public async Task ConnectAdoPatAsyncShouldRejectMissingOrganizationAsync()
    {
        var body = new ConnectPatRequest { Token = "ado_pat" };

        await Assert.ThrowsAsync<BadRequestException>(() => _controller.ConnectAdoPatAsync(body));
    }
    ...
    {
        await _controller.DisconnectAdoAsync("contoso");

        await _adoIntegrationService.Received(1).DisconnectAsync(UserId, "contoso");
        await _publisher.Received(1).Publish(Arg.Is<IntegrationDisconnectedEvent>(ev =>
            ev.UserId == UserId &&
            ev.Source == DomainItemSource.Ado &&
            ev.Organization == "contoso"), Arg.Any<CancellationToken>());
